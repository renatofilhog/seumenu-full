...
function LoginContent() {
  ...
  const redirectTo = useMemo(() => {
    ...
    if (!redirect || !redirect.startsWith("/painel")) {
      return "/painel";
    }
    return redirect;
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    const rememberedLogin = loadRememberedLogin("app");
    if (rememberedLogin) {
      setFormState({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      document.cookie = `${TENANT_CONTEXT_COOKIE}=; Path=/; Max-Age=0; SameSite=Lax`;
      setAuthCookie("app", response.accessToken, rememberMe);
      if (rememberMe) {
        saveRememberedLogin("app", {
          email: formState.email,
          senha: formState.senha,
          rememberMe: true,
        });
      } else {
        clearRememberedLogin("app");
      }
      localStorage.setItem(AUTH_USER_STORAGE, JSON.stringify(response.user));
      router.replace(response.user.forcePasswordChange ? "/painel/primeiro-acesso" : redirectTo);
    } catch (err) {
      const message =
        ...
        <main className="relative z-10 flex min-h-screen flex-col px-6 pb-10 pt-10 md:mx-auto md:max-w-2xl md:justify-center">
          ...
          <section className="mt-10 rounded-[var(--radius-md)] bg-white p-6 text-[color:var(--color-gray-800)] shadow-[var(--shadow-soft-lg)]">
            ...
            <p className="mt-2 text-sm font-semibold text-[color:var(--color-gray-500)]">
              Use seu e-mail e senha cadastrados para acessar o ambiente seguro.
            </p>
